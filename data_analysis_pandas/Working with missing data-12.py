...
temp = titanic_survival["age"][age_null == False]
correct_mean_age = sum(temp) / len(temp)
...
for p_class in passenger_classes:
    pclass_rows = titanic_survival[titanic_survival["pclass"] == p_class]
    pclass_fares = pclass_rows["fare"]
    mean = pclass_fares.mean()
    fares_by_class[p_class] = mean
...
my_new_titanic_survival = titanic_survival.dropna(subset=["age","boat"])
titanic_reindexed = my_new_titanic_survival.reset_index(drop=True)
...
def non_null_count(column):
    non_null_column = pd.isnull(column)
    non_null = column[non_null_column == False]
    return len(non_null)
